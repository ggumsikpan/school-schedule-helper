...
export default function SettingsPage() {
  ...
  function handleSave() {
    if (!form.name.trim() || !form.boardUrl.trim()) { alert('이름과 게시판 URL은 필수입니다.'); return; }
    let updated: Child[];
    if (editing) {
      ...
      <main className="max-w-lg mx-auto px-4 py-4 space-y-4">
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={openAdd} className="text-sm px-3 py-1.5 bg-amber-500 text-white rounded-full font-medium active:scale-95 transition-transform">
              + 추가
            </button>
          </div>
        ...
        <button
          onClick={() => { if (confirm('저장된 시간표 데이터를 모두 삭제할까요?')) { localStorage.removeItem('ssh_schedules'); alert('초기화 완료.'); } }}
            ...
            <div className="bg-white rounded-t-3xl w-full max-w-lg mx-auto p-6 space-y-4" onClick={e => e.stopPropagation()}>
              ...
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  placeholder="예: 민준" className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-amber-400" />
              </div>
              ...
              <div>
                ...
                <input type="text" value={form.school} onChange={e => setForm(f => ({ ...f, school: e.target.value }))}
                  placeholder="예: 행복초등학교" className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-amber-400" />
              </div>
              ...
              <div className="flex gap-2">
                ...
                <div className="flex-1">
                  ...
                  <input type="text" value={form.className} onChange={e => setForm(f => ({ ...f, className: e.target.value }))}
                    placeholder="예: 1" className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-amber-400" />
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button onClick={() => setShowForm(false)} className="flex-1 py-3 border border-gray-300 text-gray-600 rounded-xl font-medium">취소</button>
                <button onClick={handleSave} className="flex-1 py-3 bg-amber-500 text-white rounded-xl font-bold active:scale-95 transition-transform">저장</button>
              </div>
            </div>
          </div>
        )}
